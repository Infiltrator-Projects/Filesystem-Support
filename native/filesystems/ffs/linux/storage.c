/* Infiltrator Filesystem Support — FFS Linux storage adapter.
 * Allocation bitmap, regular-file I/O and inode integration are one host
 * storage responsibility around the canonical FFS engine.
 */

/*
 * Project-authored Linux allocation-bitmap adapter for Amiga FFS. On-disk arithmetic and bit-selection rules are owned by
 * the canonical FFS core; this file binds those rules to Linux buffers,
...
void affs_free_block(struct super_block *sb, u32 block)
{
    ...
    mask = ifs_ffs_bitmap_bit_mask(bit_index);
    ...
    u32 *const allocated_block)
{
    ...
    for (word_index = first_word; word_index <= last_word; ++word_index) {
        ...
        if (bm->bm_free < run_length) {
            run_length = 1U;
            run_mask = ifs_ffs_bitmap_bit_mask(first);
        }
    ...
    const u32 valid_bits)
{
    ...
    for (index = 0U; index < data_word_count; ++index) {
        ...
        if (index < full_words)
            ...
            valid_mask = ifs_ffs_bitmap_valid_word_mask(partial_bits);
        else
            valid_mask = 0U;
...
static int ifs_ffs_write_begin(
    IFS_FFS_AOPS_WRITE_CONTEXT, struct address_space *mapping,
    ...
    struct folio **folio, void **fsdata)
{
    ...
    result = cont_write_begin(
        IFS_FFS_AOPS_WRITE_CONTEXT_ARG,
    ...
    struct folio **folio_out, void **fsdata)
{
    ...
    (void)IFS_FFS_AOPS_WRITE_CONTEXT_ARG;
...
static void ifs_ffs_reset_inode_private(struct inode *inode)
{
    struct affs_inode_info *info = AFFS_I(inode);
...
static time64_t ifs_ffs_inode_time(const struct affs_date *date)
{
...
struct inode *affs_iget(struct super_block *sb, unsigned long inode_number)
{
    ...
    set_nlink(inode, 1);
    inode->i_mode = 0;
    ifs_ffs_reset_inode_private(inode);
    ...
    timestamp = ifs_ffs_inode_time(&tail->change);
