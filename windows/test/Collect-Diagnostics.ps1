...
$folder = Join-Path $env:TEMP "Filesystem-Support-Diagnostics-$stamp"
...
if ($DriverService) {
    Capture-Command 'driver-service.txt' { sc.exe query $DriverService }
}
if ($DriverFile) {
    Capture-Command 'driver-file.txt' {
        Get-Item -LiteralPath $DriverFile -ErrorAction Stop |
            Format-List FullName,Length,CreationTimeUtc,LastWriteTimeUtc,VersionInfo
    }
    Capture-Command 'driver-hash.txt' {
        Get-FileHash -Algorithm SHA256 -LiteralPath $DriverFile -ErrorAction Stop |
            Format-List
    }
}
...
Capture-Command 'system-events.txt' {
    Get-WinEvent -FilterHashtable @{LogName='System'; StartTime=(Get-Date).AddHours(-24)} -ErrorAction Stop |
        Where-Object { $_.Message -match 'Service Control Manager|disk|volume|file system|filesystem' } |
...
Write-Host "Filesystem Support diagnostics collected: $zip"
